...
pub fn tool_response_msg(tool_call_id: String, content: impl Display) -> Result<ChatCompletionRequestMessage> {
...
pub fn tool_calls_msg(tool_calls: Vec<ChatCompletionMessageToolCall>) -> Result<ChatCompletionRequestMessage> {
...
pub fn first_choice(chat_response: CreateChatCompletionResponse) -> Result<ChatChoice> {
    let first_choice = chat_response.choices.into_iter().next().ok_or("no first choive?")?;
    Ok(first_choice)
}